...
from src.nn.modules.box_ops import box_cxcywh_to_xyxy, box_iou, generalized_box_giou, generalized_box_eiou, IoU_Cal
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss_giou = 1 - torch.diag(generalized_box_giou(
            box_cxcywh_to_xyxy(src_boxes),
            box_cxcywh_to_xyxy(target_boxes)))
        losses['loss_giou'] = loss_giou.sum() / num_boxes
        losses['loss_wiou'] = IoU_Cal.WIoU(pred=box_cxcywh_to_xyxy(src_boxes), target=box_cxcywh_to_xyxy(target_boxes)).mean()
        losses['loss_siou'] = IoU_Cal.SIoU(pred=box_cxcywh_to_xyxy(src_boxes), target=box_cxcywh_to_xyxy(target_boxes)).mean()
        losses['loss_eiou'] = IoU_Cal.EIoU(pred=box_cxcywh_to_xyxy(src_boxes), target=box_cxcywh_to_xyxy(target_boxes)).mean()
